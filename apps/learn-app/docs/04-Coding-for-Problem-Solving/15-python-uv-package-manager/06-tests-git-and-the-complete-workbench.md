...
In Lesson 5, you ran `uv run pyright` on SmartNotes and watched it verify every type annotation in your code -- catching bugs before a single line executed. Pyright is the second verification tool in your workbench, after ruff. Two tools are checking your code automatically. But two tools are not enough. Code that passes linting and type checking can still do the wrong thing. And code that does the right thing today can be destroyed by a single bad edit tomorrow -- if you have no way to recover what you had before.

James learns this the hard way. It is Thursday afternoon. He has been working on a helper function for SmartNotes -- a small function called `format_title` that capitalizes the first letter of each word in a note title. He typed it carefully, tested it by eye, watched it produce the right output. Then he decides the function needs a different approach. He selects all the code, deletes it, and starts rewriting. Halfway through the rewrite, he realizes the original version was better. He reaches for Ctrl+Z. Nothing. He closed the file between versions. The undo history is gone. The original function -- the one that worked -- no longer exists anywhere.

James stares at a blank function body. He cannot remember the exact logic. He cannot recover the file. He has no tests that document what the function was supposed to do, and no version history that recorded what the function looked like before he deleted it. Two separate tools could have saved him: a test that specified the function's correct behavior, and Git that recorded the function's last working state. Without either, he has to start from memory.
