...
export function FinishStep() {
    ...
    const strengthLabel =
        !completeness ? "Calculating..." :
            (completeness.profile_completeness * 100) < 30 ? "Low" :
                (completeness.profile_completeness * 100) < 70 ? "Medium" : "High";
    ...
    return (
        ...
        >
            <div className="w-20 h-20 bg-primary/10 rounded-3xl flex items-center justify-center mb-8 shadow-inner shadow-primary/20">
                <img src="/img/logo.svg" alt="Agent Factory" className="w-10 h-10 object-contain" />
            </div>
            ...
            <p className="text-lg text-muted-foreground mb-12">
                Your digital FTE is ready to personalize your learning.
            </p>
            ...
            <div className="w-full space-y-6 mb-12 text-left">
                <div className="bg-background/50 border border-border/50 rounded-2xl p-6 shadow-sm flex flex-col gap-3">
                    <div className="flex justify-between items-center">
                        ...
                        <span className="font-semibold">5/5 Steps</span>
                    </div>
            ...
            {loading ? (
                ...
                <div className="w-full bg-accent/30 border border-border/40 rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 mb-8">
                    <div className="text-sm text-left">
                        ...
                        <span className="text-muted-foreground">Add your {formatFieldName(highestImpact)}</span>
                    </div>
                    <Button variant="secondary" size="sm" onClick={() => window.location.href = "/profile"} className="shrink-0 w-full sm:w-auto">
                        Update Profile
                    </Button>
                </div>
            ) : null}
